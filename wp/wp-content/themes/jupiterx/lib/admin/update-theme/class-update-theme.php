...
class JupiterX_Theme_Update {
	...
	public function __construct() {
		...
		add_filter( 'site_transient_update_themes', [ $this, 'check_for_update' ], 999, 1 );
		add_action( 'wp_ajax_jupiterx_clear_plugins_transient', [ $this, 'clear_plugins_transient' ] );
	}

	/**
	 * Clear plugins transient for updating Core plugin.
	 *
	 * @since 1.17.1
	 *
	 * @return void
	 */
	public function clear_plugins_transient() {
		check_ajax_referer( 'jupiterx_clear_plugins_transient' );
	 ...
	 * @since 1.11.0
	 *
	 * @param array $checked_data Update themes data.
	 * @return array Update themes data.
	 */
	public function check_for_update( $checked_data ) {
		...
		if (
			isset( $checked_data->last_checked ) &&
			( time() - $checked_data->last_checked < DAY_IN_SECONDS && ! $force_check ) ) {
			return $checked_data;
		}
		...
		foreach ( $checked_data->response as $response ) {
			if ( $response['theme'] === $theme_base && $response['new_version'] !== $theme_version ) {
				return $checked_data;
			}
		}
		...
		if ( $release_version === $theme_version ) {
			return $checked_data;
		}
		...
		set_site_transient( 'update_themes', $checked_data );
