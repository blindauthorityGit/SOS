		...
		if ( $edit ) {
			...
			if ( $image_medium ) {
				jupiterx_selfclose_markup_e(
					...
					'source',
					[
						'media'  => '(max-width: ' . $image_medium->width . 'px)',
...
function jupiterx_post_gallery( $output, $attr, $instance ) {
	...
	$gallery_div = jupiterx_open_markup(
		...
		'div',
		array(
			'class'               => "row gallery galleryid-{$id} gallery-columns-{$columns} gallery-size-{$size_class}", // Automatically escaped.
	...
	$output = apply_filters( 'gallery_style', $gallery_div ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Used in function scope.
		...
		if ( 'none' === $atts['link'] ) {
			...
			$output .= jupiterx_open_markup( "jupiterx_post_gallery_item[_{$attachment_id}]", $atts['itemtag'], [ 'class' => 'gallery-item' ] );
				...
				$output .= jupiterx_open_markup( "jupiterx_post_gallery_icon[_{$attachment_id}]", $atts['icontag'], array( 'class' => "gallery-icon {$orientation}" ) ); // Automatically escaped.
