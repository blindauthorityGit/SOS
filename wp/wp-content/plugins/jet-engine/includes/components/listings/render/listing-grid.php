...
if ( ! class_exists( 'Jet_Engine_Render_Listing_Grid' ) ) {
	...
	class Jet_Engine_Render_Listing_Grid extends Jet_Engine_Render_Base {
		...
		public static $is_top_listing = true;
		...
		public function set_is_top_listing( $value ) {
			self::$is_top_listing = $value;
		}
		...
		public function posts_template( $query, $settings ) {
			...
			if ( ! empty( $query ) ) {
				...
				$is_top_listing = self::$is_top_listing;

				if ( self::$is_top_listing ) {
					self::$is_top_listing = false;
				}
				...
				self::$is_top_listing = $is_top_listing;
